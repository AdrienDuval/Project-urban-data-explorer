"""
Transport indicator — IRIS-level scores (standalone; also reused by vivabilité).

1) ``compute_transport_score`` — buffer join, 0–10. Output feeds the **vivabilité**
   pillar (same column name in the composite). Writes ``TRANSPORT_SCORE_GOLD``.

2) ``compute_transport_indicator_score`` — API-facing 0–1 score and components.
   Feeds ``/indicators/transport`` and map layer ``TRANSPORT_INDICATOR_GOLD``.
"""
import numpy as np
