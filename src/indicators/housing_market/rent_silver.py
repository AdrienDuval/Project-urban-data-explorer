...
def process_rent_data_to_silver():
    ...
    # Charger les quartiers depuis le KML (Source Bronze)
    ...
    # Nettoyer pour n'avoir que le mapping (VAR5 <-> Code Arrondissement)
    ...
    #Jointure entre le mapping spatial et le mapping de correspondance des 
    # zones pour récupérer les codes de quartiers associés à chaque zone de calcul
    mapping_pd = mapping_pd.merge(mapping_zone, left_on="VAR5", right_on="var5", how="inner")

    # Charger les données de loyers (Bronze)
    loyers = pd.read_csv(
        RENT_DATA_CSV, 
        sep=";", 
        encoding="latin1"
    ...
    # Filtrage (Lignes où les deux colonnes sont vides/NaN)
    loyers = loyers[
